...
from utils.load_fits import load_fits_four_channel          # for loading fits images with optional normalisation
...
def load_data(data_path: str):
    ...
    # remove the sub directory Data_KiDS_Big.10 as it only has one image, and has 4 labels
    data_subdirectories.remove('Data_KiDS_Big.10')

    # store a list of lists,
    # where each inner list contains paths to each .fits file
    # where each fits file is a channel of a singular image
    ...
    labels: pd.DataFrame = pd.read_csv(os.path.join(data_path, 'kidsB_image_catalog.csv'),
    ...
    for data_subdirectory in data_subdirectories:
        ...
        for fits_file in os.listdir(current_path):
            ...
            if fits_file.endswith('.fits'):
                image_channels_paths: List[str] = []

                # append the current_path+fits_file to the list of image_paths
                image_channels_paths.append(os.path.join(current_path, fits_file))

                # find the name of the current .fits file
                fits_file_id: str = re.split('[-.]', fits_file)[1]

                # also append the other channels of the current image
                image_channels_paths.append(os.path.join(data_path, data_subdirectory, 'Public', 'Band2',
                                                         'imageSDSS_I-' + fits_file_id + '.fits'))
                image_channels_paths.append(os.path.join(data_path, data_subdirectory, 'Public', 'Band3',
                                                         'imageSDSS_G-' + fits_file_id + '.fits'))
                image_channels_paths.append(os.path.join(data_path, data_subdirectory, 'Public', 'Band4',
                                                         'imageSDSS_U-' + fits_file_id + '.fits'))

                image_paths.append(image_channels_paths)
...
def split_data(image_paths: List[List[str]], image_labels: List[int],
...
class ImageLabelGenerator(keras.utils.Sequence):
    def __init__(self, image_paths: List[List[str]], image_labels: List[int], batch_size: int = 8,
                 image_size: int = 101, no_of_channels: int = 4,
                 augmenter=None, shuffle: bool = True):
        self.image_paths: List[List[str]] = image_paths
    ...
    def __getitem__(self, index):
        ...
        for i, (image_channels_paths, image_label) in enumerate(zip(image_paths_batch, image_labels_batch)):
            image_batch[i] = load_fits_four_channel(image_channels_paths, 'ZScale')#, fits_file_normalisation) #TODO
            label_batch[i] = image_label

        # to display a 'before augmentation' image
        # import imgaug as ia
        # ia.imshow(np.hstack(image_batch))
...
if __name__ == '__main__':
    ...
    if model_train_load == 'train':
        ...
        if not os.path.exists('models'):
            # if not, create it
            os.makedirs('models')
        model_name: str = 'ground_' + model_name + '_' + str(no_of_epochs) + 'epochs'
